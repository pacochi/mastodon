  ...
  describe 'callbacks' do
    describe 'statuses_count' do
      context 'on create' do
        ...
        it 'updates statuses_count' do
          is_expected.to change {
            alice.statuses_count
          }.from(0).to(1)
        end
      end
      ...
      context 'on destroy' do
        ...
        context 'on multiple destroy' do
          subject do
            -> { Status.where(id: [status_1.id, status_2.id]).destroy_all }
          end

          let!(:status_1) { status }
          let!(:status_2) { Fabricate(:status, account: alice) }

          it 'updates statuses_count' do
            is_expected.to change {
              alice.reload.statuses_count
            }.from(2).to(0)
          end
        end
      end
    ...
    describe 'reblogs_count' do
      ...
      context 'on create' do
        subject do
          -> { Fabricate(:status, reblog: status) }
        end

        it 'updates reblogs_count' do
          is_expected.to change {
      ...
      context 'on destroy' do
        ...
        context 'on multiple destroy' do
          subject do
            -> { Status.where(id: [reblog_1.id, reblog_2.id]).destroy_all }
          end

          let!(:reblog_1) { reblog }
          let!(:reblog_2) { Fabricate(:status, reblog: status) }

          it 'updates reblogs_count' do
            is_expected.to change {
              status.reload.reblogs_count
            }.from(2).to(0)
          end
        end
      end
