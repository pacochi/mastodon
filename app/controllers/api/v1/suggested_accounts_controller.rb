...
class Api::V1::SuggestedAccountsController < ApiController
  ...
  def index
    ...
    @accounts = suggested_accounts(current_user.account).shuffle(seed).per(limit).page(params[:page]).all

    media_attachments = popular_media_attachments(@accounts)
    @media_attachments_map = media_attachments.group_by(&:account_id)

    next_path = api_v1_suggested_accounts_url(seed: seed, page: params[:page].to_i + 1) if @accounts.size == limit
